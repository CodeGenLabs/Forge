...
| W3 | applied & verified | **Delivered & applied**: `docs/measurements/monorepo-handover.patch` applied to `D:/git/studio-monorepo` on user approval; 10/10 tools tests pass, contract check OK, 40/40 redact tests pass |
...
3. *W3 Driver test*: In `studio-monorepo`, driver registration was previously embedded inline inside `buildEngine()` which initialized a SQLite database and vault. A conformance test in `tools/__tests__/` could not test driver resolution without spinning up workspace disk state. Extracting `registerAllDrivers()` in `@acme/host` was required to make the contract verifiable in isolation.
...
1. **Historical drift census (n=3)**:
   ...
   - Run 3 (`studio-monorepo`): Simulated drift on Bob's commit. Attributed and confirmed. Code/claim changed: **0 lines**.
...
- ~~The `docker-dev-connections.test.ts` can import `@acme/driver-core` directly via alias.~~ **Corrected**: Root `tsconfig.json` includes only `src` and `vite.config.ts`, so `tsconfig-paths` in vitest cannot resolve package aliases for files under `tools/`. Changed to relative package paths `../../packages/...`.
...
## W3 — The monorepo patch

### What Was Delivered and Applied
1. One patch file against `D:/git/studio-monorepo`, generated from an isolated clone (`--no-hardlinks`) at `docs/measurements/monorepo-handover.patch`.
2. Applied directly to `D:/git/studio-monorepo` following user approval.
...
2. **Driver-registry conformance test (`PIT-seeded-driver-must-be-registered`)**:
   - Extracted `registerAllDrivers()` in `packages/host/src/engine.ts` and exported it from `@acme/host`.
...
### Suite Verification on `D:/git/studio-monorepo`
...
### What Shipped
- `docs/measurements/monorepo-handover.patch`.
